...
static hive::ModuleRegistry s_ModuleRegistry;

int main()
{
    hive::ModuleRegistry &moduleRegistry = hive::ModuleRegistry::GetInstance();

    hive::LogManager logManager;
    hive::ConsoleLogger consoleLogger{logManager};
    ...
    if (terra::Window::BackendInitialize())
    {
        ...
        swarm::WindowHandle windowHandle;
        windowHandle.wayland.m_Surface = (wl_surface *) handle.windowHandle;
        windowHandle.wayland.m_Display = (wl_display *) handle.displayHandle;
        swarm::GraphicDevice gfxdevice{windowHandle};
