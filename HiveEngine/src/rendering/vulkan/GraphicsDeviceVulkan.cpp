...
bool hive::vk::GraphicsDevice_Vulkan::CreatePipeline(const PipelineDesc &desc, Pipeline &pipeline)
{
    if (available_pipeline.empty()) return false;
    const u32 id = available_pipeline.front();
    available_pipeline.pop();

    std::vector<VkPipelineShaderStageCreateInfo> stages;
    for(auto stage : desc.shaders_stages)
    {
        VkShaderModule module = static_cast<VkShaderModule>(stage.local_data);
        switch (stage.desc.stage)
        {
            case ShaderStage::VERTEX:
                stages.push_back(create_stage_info(module, StageType::VERTEX));
                break;
            case ShaderStage::FRAGMENT:
                stages.push_back(create_stage_info(module, StageType::FRAGMENT));
                break;
        }
    }

    VkPolygonMode cull_mode{};
    switch (desc.cull_mode)
    {
        case PipelineDesc::CullMode::FILL:
            cull_mode = VK_POLYGON_MODE_FILL;
            break;
        case PipelineDesc::CullMode::LINE:
            cull_mode = VK_POLYGON_MODE_LINE;
            break;
        case PipelineDesc::CullMode::POINT:
            cull_mode = VK_POLYGON_MODE_POINT;
            break;
    }
    ...
    for(auto layout : desc.layouts)
    {
        switch (layout.type)
        {
            case DescriptorType::UNIFORM_BUFFER:
                ...
                layout_builder.addBinding(layout.binding_location, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, get_vulkan_stage(layout.stage), 1);
                break;
            case DescriptorType::SAMPLER2D:
                pool_builder.addPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, MAX_FRAME_IN_FLIGHT);
                layout_builder.addBinding(layout.binding_location, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, get_vulkan_stage(layout.stage), 1);
                break;
        }
    }
    ...
    auto &pipeline_data = pipelines[id];
    ...
    {
        if(!pipeline_data.pool->allocateDescriptor(pipeline_data.layout->getDescriptorSetLayout(), pipeline_data.descriptor_sets[i])) return false;
    }

    if(!create_graphics_pipeline(device_, render_pass_, stages.data(), stages.size(), MAX_FRAME_IN_FLIGHT, cull_mode, pipeline_data))
    {
        return false;
    }
